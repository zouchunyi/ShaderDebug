    ...
    [MovedFrom("UnityEngine.Rendering.LWRP")] public abstract class ScriptableRenderer
    {
...
#if FRAGMENG_DEBUG
        RenderTargetHandle m_CameraDebugTarget;
#endif
...
#if FRAGMENG_DEBUG
        public void ConfigureDebugTarget(RenderTargetHandle debugTarget)
        {
            m_CameraDebugTarget = debugTarget;
        }
#endif
        ...
        void ExecuteRenderPass(ScriptableRenderContext context, ScriptableRenderPass renderPass, ref RenderingData renderingData)
        {
...
#if FRAGMENG_DEBUG
            bool useMRT = true;
            if (cameraData.camera.name.Equals("SceneCamera") || cameraData.camera.name.Equals("Preview Scene Camera"))
            {
                useMRT = false;
            }
            cmd.GetTemporaryRT(m_CameraDebugTarget.id, Screen.width, Screen.height, 0, FilterMode.Point, RenderTextureFormat.ARGBFloat);
            cmd.SetGlobalTexture("_CameraDebugTexture", m_CameraDebugTarget.Identifier());
#endif
            ...
            if (passColorAttachment == m_CameraColorTarget && !m_FirstCameraRenderPassExecuted)
            {
                ...
                if (useMRT)
                {
                    SetRenderTargetWithDebug(cmd, m_CameraColorTarget, m_CameraDepthTarget, m_CameraDebugTarget.Identifier(), clearFlag,
            ...
            else if (passColorAttachment != m_ActiveColorAttachment || passDepthAttachment != m_ActiveDepthAttachment)
            {
                ...
                if (useMRT)
                {
                    SetRenderTargetWithDebug(cmd, passColorAttachment, passDepthAttachment, m_CameraDebugTarget.Identifier(), renderPass.clearFlag, renderPass.clearColor);
                }
                else
                {
            ...
            context.ExecuteCommandBuffer(cmd);
#if FRAGMENG_DEBUG
            cmd.ReleaseTemporaryRT(m_CameraDebugTarget.id);
#endif
            CommandBufferPool.Release(cmd);
...
#if FRAGMENG_DEBUG
        internal static void SetRenderTargetWithDebug(CommandBuffer cmd, RenderTargetIdentifier colorAttachment, RenderTargetIdentifier depthAttachment, RenderTargetIdentifier debugAttachment, ClearFlag clearFlag, Color clearColor)
        {
            m_ActiveColorAttachment = colorAttachment;
            m_ActiveDepthAttachment = depthAttachment;
            ...
            SetRenderTargetWithDebug(cmd, colorAttachment, colorLoadAction, RenderBufferStoreAction.Store,
                depthAttachment, depthLoadAction, RenderBufferStoreAction.Store, clearFlag, clearColor, dimension, debugAttachment);
        }

        static void SetRenderTargetWithDebug(
            CommandBuffer cmd,
            ...
            RenderTargetIdentifier debugAttachment)
        {
            ...
            {
                if (dimension == TextureDimension.Tex2D)//2 - true
                    CoreUtils.SetRenderTargetWithDebug(cmd, colorAttachment, depthAttachment, debugAttachment,
